...
function Side({ side, won, owner, verdict }: { side: TradeSide; won: boolean; owner: OwnerLite; verdict: string }) {
  return (
...
export default function TradeCard({ trade, owners, rank }: { trade: Trade; owners: Map<string, OwnerLite>; rank?: number }) {
  ...
  return (
    <div className="card p-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <Side side={trade.winner} won owner={o(trade.winner.ownerKey)} verdict="Won" />
        <Side side={trade.loser} won={false} owner={o(trade.loser.ownerKey)} verdict="Lost" />
      </div>
    </div>
  );
}
